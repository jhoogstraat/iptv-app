...
- Implementation status (reviewed 2026-07-16): `Tabs` provides stable non-localized customization IDs, `ContentView` renders the `sidebarAdaptable` shell, and every data-backed top-level surface is mounted through `SessionGuard`, including Search. Movies, Series, and Live use native category `NavigationLink`s before media/channel destinations, so system back controls and interactive back gestures preserve the category landing context. Movies and Series place `SessionGuard` outside the `NavigationStack`, so category, movie, series, and episode destinations inherit the same `Session`. Detail routes use `MediaDetailDestination`; player presentation is wired through root full-screen presentation, visionOS window presentation, and macOS `PlayerWindow`. Dismissing the expanded controller leaves off-device playback alive for the compact now-playing bar to restore, while dismissing local playback closes the session so it cannot continue without controls.
...
- Player presentation is modeled by `Player.presentation` and is wired at the app root through `IPTVApp`, `withVideoPlayer()`, and macOS `PlayerWindow`. `PlaybackDestinationCoordinator` independently owns destination and renderer placement, while `ExternalNowPlayingBar` keeps off-device playback reachable from the tab shell.
